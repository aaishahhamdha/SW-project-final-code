...
const Dashboard = ({ showTour }) => {
  ...
  const renderCollaboratedDBs = () => {
    if (collaboratedDatabases.length === 0) {
      return <p className="NoDB">No collaborations yet.</p>;
    }
  ...
  const renderDatabaseLinks = () => {
    if (connectedDatabases.length === 0) {
      return <p className="NoDB">No connected databases yet.</p>;
    }
    return connectedDatabases.map((db) => (
      <div key={db.id} className="btn1">
